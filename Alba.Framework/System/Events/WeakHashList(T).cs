...
/// <remarks>From https://github.com/AvaloniaUI/Avalonia/blob/master/src/Avalonia.Base/Utilities/WeakHashList.cs</remarks>
internal class WeakHashList<T> where T : class
{
    public const int DefaultArraySize = 8;

    private static readonly Stack<PooledList<T>> _ListPool = new();
    ...
    public void Add(T item)
    {
        ...
        if (_arrCount < _arr.Length) {
            _arr[_arrCount] = new(item);
            _arrCount++;
            return;
        }

        for (var c = 0; c < _arrCount; c++) {
            if (!_arr[c]!.TryGetTarget(out _)) {
                _arr[c] = new(item);
                return;
            }
        }

        _dic = new(KeyComparer.Instance);
    ...
    public void Remove(T item)
    {
        if (_arr != null) {
            ...
            var strongKey = Key.MakeStrong(item);
            if (_dic.TryGetValue(strongKey, out var cnt) && cnt > 1) {
                _dic[strongKey] = cnt - 1;
                return;
            }
        ...
        if (_arr != null) {
            ...
            for (var c = 0; c < _arrCount; c++) {
                if (_arr[c]?.TryGetTarget(out var target) == true) {
                    pooled ??= factory?.Invoke() ?? (_ListPool.Count > 0 ? _ListPool.Pop() : [ ]);
                    pooled.Add(target);
                }
                else {
                    _arr[c] = null;
    ...
    private class KeyComparer : IEqualityComparer<Key>
    {
        public bool Equals(Key x, Key y)
        {
            ...
            if (x.Strong != null) {
                ...
                if (x.Weak == null || !x.Weak.TryGetTarget(out var xTarget))
